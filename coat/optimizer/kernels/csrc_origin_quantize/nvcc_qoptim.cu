...
void myKernelLauncher(float* params, float* grads,
                      __nv_fp8_e4m3* exp_avg, float* scale_exp_avg,
                      __nv_fp8_e4m3* exp_avg_sq, float* scale_exp_avg_sq,
                      float beta1, float beta2, float lr, float wd, float eps, int step, int qgroup_size,
                      int M, int N) {
    ...
    if (algo == fp8_adamw) {
        ...
        printf("Yes!\n");
        fp8_adamw_csrc<float><<<gridDim, blockDim>>>(params, grads, exp_avg, scale_exp_avg, exp_avg_sq, scale_exp_avg_sq,
                                              beta1, beta2, lr, wd, eps, step, qgroup_size, M * N, int(floor(M * N / 128.)));
...
float testMaxError(
    void (*myGPUKernel) (float*, float*, __nv_fp8_e4m3*, float*, __nv_fp8_e4m3*, float*,
    ...
    myGPUKernel(d_p, d_g, d_m, d_sm, d_v, d_sv, beta1, beta2, lr, wd, eps, step, qgroup_size, M, N);
