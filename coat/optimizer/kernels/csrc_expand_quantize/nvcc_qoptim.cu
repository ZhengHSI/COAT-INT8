...
void fp8_adamw_cpu(
    ...
    float * fp_exp_avg,
    float * fp_exp_avg_sq,
    float beta1, float beta2, float lr, float wd, float eps, int step, int qgroup_size, int M, int N) {

    for (int idx = 0; idx < M * N; idx++){
        fp_exp_avg[idx] = beta1 * fp_exp_avg[idx] + (1 - beta1) * grads[idx];
        fp_exp_avg_sq[idx] = beta2 * fp_exp_avg_sq[idx] + (1 - beta2) * grads[idx] * grads[idx];

        const float correction1 = 1.0f - powf(beta1, step);
        const float correction2_sqrt = sqrtf(1.0f - powf(beta2, step));

        float denom = (sqrtf(fp_exp_avg_sq[idx]) / correction2_sqrt + eps) * correction1;
...
template<typename T>
void printFloatArrayToFile(T* array, int M, int N, const std::string& outputFileName) {
    ...
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            int index = i * N + j;
            outputFile << std::setw(10) << std::right << std::fixed << std::setprecision(6) << (float)array[index] << " ";
            if (j == N - 1) {
                outputFile << "\n";
            }
        }
...
void myKernelLauncher(float* params, float* grads,
                      __nv_fp8_e4m3* exp_avg, float* scale_exp_avg, float* expand_exp_avg, float* sqrtminmax_exp_avg,
                      __nv_fp8_e4m3* exp_avg_sq, float* scale_exp_avg_sq, float* expand_exp_avg_sq, float* sqrtminmax_exp_avg_sq,
                      float beta1, float beta2, float lr, float wd, float eps, int step, int qgroup_size, int expand_min,
                      int M, int N) {
    ...
    if (algo == fp8_adamw) {
        ...
        printf("Yes!\n");
        fp8_adamw_csrc<float><<<gridDim, blockDim>>>(params, grads,
                                                     ...
                                                     beta1, beta2, lr, wd, eps, step, qgroup_size, expand_min, M * N, int(floor(M * N / 128.)));
        cudaError_t error = cudaGetLastError();
        if (error != cudaSuccess) {
            std::cout << "CUDA error occurred in kernel launch: " << cudaGetErrorString(error) << std::endl;
            return;
        }
        printf("Finish!\n");
    }
...
float testMaxError(
    void (*myGPUKernel) (float*, float*, __nv_fp8_e4m3*, float*, float*, float*, __nv_fp8_e4m3*, float*, float*, float*, // tensor input
                      float, float, float, float, float, int, int, int, // float and int input
                      int, int), // M and N
    int M, int N) {
    ...
    cudaError_t error = cudaGetLastError();
    if (error != cudaSuccess) {
        std::cout << "CUDA error occurred in data copy: " << cudaGetErrorString(error) << std::endl;
        return 0.;
    }
    ...
    fp8_adamw_cpu(h_p, h_g, h_fp_m, h_fp_v, beta1, beta2, lr, wd, eps, step, qgroup_size, M, N);

    if (error != cudaSuccess) {
        std::cout << "CUDA error occurred in data initialization: " << cudaGetErrorString(error) << std::endl;
        return 0.;
    }

    myGPUKernel(d_p, d_g, d_m, d_sm, d_cpd_m, d_sqrtmm_m, d_v, d_sv, d_cpd_v, d_sqrtmm_v, beta1, beta2, lr, wd, eps, step, qgroup_size, expand_min, M, N);
    ...
    for (int i = 0; i < M * N; i++) {
        hd_fp_m[i] = pow((float)hd_m[i] * hd_sm[int(floor(i / 128.))], 1 / hd_cpd_m[int(floor(i / 128.))]) * hd_sqrtmm_m[int(floor(i / 128.))];
        hd_fp_v[i] = pow((float)hd_v[i] * hd_sv[int(floor(i / 128.))], 1 / hd_cpd_v[int(floor(i / 128.))]) * hd_sqrtmm_v[int(floor(i / 128.))];
    }
    ...
    printFloatArrayToFile(hd_sqrtmm_m, 1, int(ceilf(M * N / 128.)), "GPU_sqrtmm_m.txt");
    printFloatArrayToFile(hd_sqrtmm_v, 1, int(ceilf(M * N / 128.)), "GPU_sqrtmm_v.txt");
...
int main() {
    ...
    float max_error = testMaxError(
        myKernelLauncher<fp8_adamw>, M, N
    );
